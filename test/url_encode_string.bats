...
@test "URL encoding test for simple string" {
  result="$(./assets/encode_string.sh "hello world")"
...
@test "URL encoding test for empty string" {
  result="$(./assets/encode_string.sh "")"
  [ "$result" = "" ]
}
...
@test "URL encoding test for reserved characters" {
  result="$(./assets/encode_string.sh "hello?world#test")"
  [ "$result" = "hello%3Fworld%23test" ]
}

@test "URL encoding test with no parameters" {
  run ./assets/encode_string.sh
  [ "$status" -eq 1 ]
  [ "${lines[0]}" = "Usage: ./assets/encode_string.sh <string>" ]
}

@test "URL encoding test with more than one parameter" {
  run ./assets/encode_string.sh "hello" "world"
  [ "$status" -eq 1 ]
  [ "${lines[0]}" = "Usage: ./assets/encode_string.sh <string>" ]
}